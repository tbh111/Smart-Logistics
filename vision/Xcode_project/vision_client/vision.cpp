...
void Vision::detectLight(Mat& image, int markerId, vector<Point2f> markerCorner, Point2f& left_corner, Point2f& right_corner, Mat& light_roi){
	...
	pt1.x = mid.x - 0.1 * markerLen;
	pt2.x = mid.x + 0.1 * markerLen;
	pt1.y = left_corner.y - 1.1* markerLen;
...
int Vision::lightJudge(Mat& image, Mat& light){
	...
	if(contours.size() > 0){
		...
		if(largest_area > 3){
			...
			Point center(rect.center.x, rect.center.y);
			if(center.y < image.rows/3) {
				cout << "red" << endl;
				return 0;
			}
			else if(center.y < 2*image.rows/3 && center.y > image.rows/3) {
				cout << "yellow" << endl;
				return 1;
			}
			else{
...
//					Point2f pt(a, b);
//
//					if(pointPolygonTest(max_contour, pt, false) == 1){
//						circle(light, Point(a, b), 0.01, Scalar(0,255,0));
//						H = temp.at<Vec3b>(a,b)[0];
//						S = temp.at<Vec3b>(a,b)[1];
//						V = temp.at<Vec3b>(a,b)[2];
//						printf("a:%d b:%d %d %d %d\n", a, b, temp.at<Vec3b>(a,b)[0],temp.at<Vec3b>(a,b)[1],temp.at<Vec3b>(a,b)[2]);
...
void Vision::estimatePose_3(Mat& image, Mat& light_roi){
	...
	readArucoMap("aruco_test.txt", totalIds, totalPts); // aruco_video为视频中的map   aruco_test为打印的map
